...
#if defined(NDEBUG)
# error "Gepardcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf3cd8715bfd0515fea9778b868ed013a6cfc1b5e3a65357daeec460995676d2d");//Fő block
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Gepardcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Gepardcoin Signed Message:\n";
...
{
    // Gepardcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Gepardcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Gepardcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Gepardcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gepardcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Gepardcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0e1af645d47a9ea7cea2807d48c248d8e2fd0e1ab9682d79bb67bba085179219");//other that fontos
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Finance Minister Inaugurates HQ of International Investment Bank/2021/03/02/";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0488b0b72c82f317e1d3a7ef08e28ff041512f17c38e373c7e5927bd522a63bc20f5539ec33a3184246f5fec6016183168c6b25138917bd6088f3510b283f9f48a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1614684556;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084635052;

        if (fTestNet)
        {
            block.nTime    = 1614685556;

            block.nNonce   = 385674794;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x52be0bb3034b988d55d06af111acf20ac8cd12b064786ee9378faa8a04cf56f0"));//This fontos
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Gepardcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GepardcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GepardcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GepardcoinMiner(CWallet *pwallet)
{
    printf("GepardcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gepardcoin-miner");
    ...
    try { loop {
        ...
        printf("Running GepardcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GepardcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GepardcoinMiner, pwallet));
}
